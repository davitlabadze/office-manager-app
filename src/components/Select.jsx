  ...
  return (
    <div className='relative mt-12'>
      <button
        onClick={show}
        type='button'
        className='relative w-full py-4 pl-3 pr-10 text-left bg-gray-200 border border-gray-300 rounded-md shadow-sm cursor-default focus:outline-none sm:text-sm'
      >
        <span className='block truncate'>
          {selected ? selected.position : t(title)}
        </span>
        ...
        <ul className='absolute z-10 w-full py-1 overflow-auto text-base bg-white rounded-md shadow-lg shadow-3xl max-h-60 ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
          {data.map((position) => (
            <li
              ...
              className={`
            ${selected.id === position.id ? `bg-blue-200` : ''} 
